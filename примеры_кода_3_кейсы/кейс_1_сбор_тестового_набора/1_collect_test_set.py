...
# Улучшенный алгоритм: использует производную для точных границ + проверку возврата к уровню шума
    ...
    with np.load(filepath) as data:
        ...
        for idx, (is_above, is_noise) in enumerate(zip(above_current_threshold, at_noise_level)):
            if is_above and not in_impulse:
                ...
                # Ищем точную границу по производной
                end_idx = idx
        ...
        for start, end in zip(impulse_starts, impulse_ends):
            if end - start >= min_duration:  # Фильтруем короткие импульсы
                # Добавляем padding в начале и конце
