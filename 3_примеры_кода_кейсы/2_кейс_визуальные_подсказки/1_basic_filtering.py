"""
Простая фильтрация данных по амплитуде и площади под кривой.
Разбивает данные на сегменты и проверяет наличие значимых сигналов.
"""
...
def simple_filter(batch_data):
    ...
    # Проверяем по площади в сегментах
...
try:
    ...
    # Применение фильтрации к временным промежуткам
    for batch in batches:
        if simple_filter(batch['i']):
            print(f"Временной промежуток номер {batch['batch_index']}: содержит потенциальные импульсы")
            # Здесь можно добавить обработку временного промежутка (визуализация, анализ и т.д.)
        else:
